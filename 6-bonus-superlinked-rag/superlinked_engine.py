...
class SuperlinkedEngine(metaclass=SingletonMeta):
    ...
    def __build_sources(self) -> None:
        if self._use_redis:
            self.article_source = RestSource(article, parser=self.article_parser)
            self.repository_source = RestSource(repository, parser=self.repository_parser)
            self.post_source = RestSource(post, parser=self.post_parser)
        else:
            self.article_source = InMemorySource(
                article, parser=self.article_parser
            )
            self.repository_source = InMemorySource(
                repository, parser=self.repository_parser
            )
            self.post_source = InMemorySource(post, parser=self.post_parser)
         
    def __build_executor(self) -> None:
        if self._use_redis:
            vector_database = RedisVectorDatabase(
                "localhost", # (Mandatory) This is your redis URL without any port or extra fields
            ...
            self.executor = RestExecutor(
                ...
                queries=[RestQuery(RestDescriptor("query"), self.get_article_query(filters={}))],
